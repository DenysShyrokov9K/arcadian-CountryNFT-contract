...
describe("BallCountry", function () {
    ...
    before(async () => {
        ...
        payToken = await (await ethers.getContractFactory("BallCountry")).deploy()
    ...
    it('test_deposit_with_rightValue_then_success', async () => {
        ...
        await payToken.connect(signer1).burn(4);
        await payToken.connect(signer1).upgrade(1,4,4,4,4,{value: '9000000000000000000'});
        console.log(await payToken.connect(signer4).currentUri())
        console.log(await payToken.connect(signer1).userTokenIds(signer1.address,0));
    })
